...
\cite<MONC,>{Brown_2020} \todo{which version? Is there a better reference?}.
...
temperature tendency set as \todo{double check in code for both WRF and MONC}
...
MONC was run on a \todo{X $\times$ X km$^2$} domain for grid spacings of 1 km, 500 m, and 250 m. We used an idealized treatment of evaporation in the sense that a
...
the fixed Chua parameters. \todo{Add MONC settings including parameterisations
used and method of perturbation application.}
...
Constant perturbations were applied to potential temperature \todo{correct to
say potential temperature not just temperature for both models?} or water vapour
...
\begin{figure}[pth]
    ...
    MONC, the plots show timeseries of spatial mean \todo{correct?} columnar
    water vapour by grid spacing. RCE runs were computed until precipitable
    water content/columnar water content stabilised and the model was at RCE. At
    this point the perturbations were introduced and the model was run until all
    perturbed runs were also at RCE. The parts of the time axes highlighted in
    green show the ``RCE region'' over which mean profiles were calculated for
    comparison \todo{add RCE regions for MONC}. Note that to reduce the number
    of colours required, positive and negative perturbations per variable and
    perturbation pressure use the same colour. \todo{Why do MONC perturbation
    runs also run in the RCE period and show slightly different results there?}
    \todo{Do we agree that WRF is sufficiently settled into RCE at 100 m grid
    spacing or is more simulation time required?}}
    \label{fig:rce_pw}
\end{figure}
...
\begin{figure}[pth]
    ...
    mixing ratios are in g kg-1?} \todo{What's the explanation for large q values near the surface for MONC?} \todo{Is there a unit problem with the MONC values of qice?}}
...
of meaningful organisation in these fixed-radiation setups \todo{is this also true for MONC?}.
...
\begin{figure}[pth]
    ...
    perturbation at 850 hPa. \todo{Note negative reponses for T at 850 in MONC files appear to be in 1000 * g kg-1.}}
    \label{fig:tpert_850}
\end{figure}
...
ice mixing ratio responses?}. MONC responds more strongly for graupel, however,
especially in response to higher level perturbations where non-linearities are
evident.
